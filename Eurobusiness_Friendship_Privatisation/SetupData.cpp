...
std::vector<Pole*> SetupData::SetupAreas()
{
	std::vector<Pole*> entries;
...
Nieruchomosc* SetupData::addProperty(std::vector<std::string> data)
{
	...
	Nieruchomosc* tmp = new Nieruchomosc(name, market_value, dev_values);
	
	return tmp;
}

PoleUsluga* SetupData::addUtilityField(std::vector<std::string> data)
{
	std::string name = data[1];
	...
	PoleUsluga* tmp = new PoleUsluga(name, operational_value);
	
	return tmp;
}

NieruchomoscOgraniczona* SetupData::addLtdProperty(std::vector<std::string> data)
{
	std::string name = data[1];
	unsigned market_value = SetupData::strToInt(data[2]);
	...
	NieruchomoscOgraniczona* tmp = new NieruchomoscOgraniczona(name, market_value, prices);
	return tmp;
}
