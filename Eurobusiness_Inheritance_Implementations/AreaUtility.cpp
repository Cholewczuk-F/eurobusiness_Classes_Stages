...
AreaUtility::AreaUtility(std::string type, std::string name, int valuePassed) : Area("AreaUtility", name)
{
	value = valuePassed;
}

unsigned AreaUtility::getId()
{
	return id;
}
//returns an id of property

std::string AreaUtility::getType()
{
	return type;
}

std::string AreaUtility::getName()
{
	return name;
}

std::string AreaUtility::getInfo()
{
...
Player* AreaUtility::interact(Player* playerAtProperty)
{
	//Bank::acquirePayment();
}
